...
(function(global) {
    ...
    global.OmniQR = {
        renderCanvas: function(text, canvas, color = '#000000', bgColor = '#ffffff', margin = 2) {
            var qr = new QRCode(0, 1); // Auto type, Medium EC
            qr.addData(text);
            qr.make();
            var count = qr.getModuleCount();
            var ctx = canvas.getContext('2d');
            var size = canvas.width;
            var cellSize = size / (count + 2 * margin);
            ...
            for (var r = 0; r < count; r++) {
                for (var c = 0; c < count; c++) {
                    if (qr.isDark(r, c)) {
                        ctx.fillRect((c + margin) * cellSize, (r + margin) * cellSize, cellSize + 0.5, cellSize + 0.5);
                    }
                }
            }
        },
        generateSVG: function(text, color = '#000000', bgColor = '#ffffff', margin = 2) {
            var qr = new QRCode(0, 1);
            qr.addData(text);
            qr.make();
            var count = qr.getModuleCount();
            var total = count + 2 * margin;
            var rects = [];

            for (var r = 0; r < count; r++) {
                for (var c = 0; c < count; c++) {
                    if (qr.isDark(r, c)) {
                        rects.push(`<rect x="${c + margin}" y="${r + margin}" width="1" height="1" fill="${color}"/>`);
                    }
                }
            }
